...
builtins.intersectAttrs super {
  ...
  mysql = dontCheck (addBuildTool pkgs.buildPackages.libmysqlclient super.mysql);
  ...
  SDL = addBuildTool pkgs.buildPackages.SDL super.SDL;
  SDL-image = with pkgs.buildPackages; addBuildTools [ SDL SDL_image ] super.SDL-image;
  SDL-ttf =
    with pkgs.buildPackages;
    addBuildTool [
      SDL
  ...
  SDL-gfx = addBuildTool pkgs.buildPackages.SDL super.SDL-gfx;
  ...
  cpython = doJailbreak (addBuildTool pkgs.python3 super.cpython);

  inline-python = addBuildTool pkgs.python3 super.inline-python;
  ...
  # Note cabal2nix hides this for a few tools, e.g.
  #   a) if a pkg has 'build-depends: hspec-discover', cabal2nix will re-use it
  #   b) if a pkg has 'build-depends: hspec', its dependency 'hspec-discover' is propagated
  # Neither of these workarounds work when cross compiling or when 'strictDeps = true'
